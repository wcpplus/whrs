...
// Props
interface Props {
  modelValue?: string[]; // e.g. ['110000', '110100']
  placeholder?: string;
  disabled?: boolean;
...
const emit = defineEmits<{
  ...
  (e: 'change', names: string[], codes: string[]): void;
}>();
...
watch(
  () => props.modelValue,
  (newVal) => {
    if (newVal && JSON.stringify(newVal) !== JSON.stringify(selectedValue.value)) {
...
const handleChange = (value: string[]) => {
  ...
  const names = value.map((code) => {
    for (const prov of cityOptions) {
      if (prov.value === code) return prov.label;
      if (prov.children) {
        const city = prov.children.find(c => c.value === code);
        if (city) return city.label;
      }
    }
    return code;
  });
  emit('change', names, value);
};
</script>
