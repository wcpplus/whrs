/**
 * 将对象存储到 sessionStorage 中，并设置10分钟有效期
 * @param key - 存储的键名
 * @param obj - 要存储的对象
...
const put = (key: string, obj: object, cacheMinute?: number): void => {
  try {
    ...
    // 捕获可能的异常并输出错误信息
    console.error("sessionStorage存储错误:", error);
  }
};

/**
 * 从 sessionStorage 中读取并反序列化对象，若已过期则自动清除并返回 null
 * @param key - 要读取的键名
 * @returns 返回解析后的对象，如果不存在或已过期则返回 null
 */
const get = (key: string): object | null => {
  try {
    // 从 sessionStorage 获取字符串数据
    ...
    // 捕获可能的异常并输出错误信息
    console.error("sessionStorage读取错误:", error);
    return null;
  }
};

/**
 * 清除 sessionStorage 中的某个键或全部数据
 * @param key - 要清除的键名（可选）
 */
