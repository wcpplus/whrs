  ...
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      ...
      <el-form-item label="关键字：" prop="keyid" v-if="isShowform.keyid"
        :rules="v.getRegExpRules(v.ValidRxs.indexKey, true, 1, 256)">
      ...
      <el-form-item label="字典ID:" prop="entityid" v-if="isShowform.entityid" :rules="v.getRules(true, 0, 32)">
...
const initCreatForm = (formParam: FormParam) => {
  ...
  form.entityid = formParam.id as string;
...
const viewData = async (id: string) => {
  farm2Request.submit('api/dictype/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/dictype', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/dictype/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
