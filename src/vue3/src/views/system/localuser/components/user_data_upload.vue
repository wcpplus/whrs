<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form v-loading="isLoading" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      <File_upload_box :multiple="false" :max-size="1024 * 1024" model="FILE" :ex-names="['xls']"
        :error-handle="errorHandle" :success-handle="successHandle">
      </File_upload_box>
    </el-form>
  </el-drawer>
</template>
...
const successHandle = (response: DataResponse) => {
  Farm2Request.submit('api/localuser/import', RequestTypeEnum.post, isLoading, { id: (response.data as { id: string }).id }).then(() => {
    notice.bySuccess("上传成功!");//提示成功
    eventBus.emit(PageEvent.data_do_query);//刷新列表
    formCtrl.isDrawerOpen = false;
  }).catch((error: Error) => {
    notice.byError(error.message);
  });
}
