<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false" size="500px">
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      ...
      <el-form-item label="业务id:" prop="appid" v-if="isShowform.appid" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.appid" />
      </el-form-item>
      <el-form-item label="业务类型：" prop="type" v-if="isShowform.type" :rules="v.getRules(false)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/resourcefileregiste/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/resourcefileregiste', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/resourcefileregiste/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
