  ...
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto"
      ...
      <el-form-item label="名称：" prop="name" v-if="isShowform.name" :rules="v.getRules(true, 0, 128)">
        <el-input v-model="form.name" autosize type="textarea" placeholder="多个职级用逗号分隔" />
      </el-form-item>
      <el-form-item label="编码：" prop="keyid" v-if="isShowform.keyid" :rules="v.getRules(false, 0, 128)">
        <el-input v-model="form.keyid" autosize type="textarea" />
      </el-form-item>
      <el-form-item label="序列类型:" prop="tracktype" v-if="isShowform.tracktype" :rules="v.getRules(true, 0, 16)">
      ...
      <el-form-item label="最低工资:" prop="minsalary" v-if="isShowform.minsalary" :rules="v.getRules(false)">
        <el-input-number v-model="form.minsalary" />
      </el-form-item>
      <el-form-item label="最高工资:" prop="maxsalary" v-if="isShowform.maxsalary" :rules="v.getRules(false)">
        <el-input-number v-model="form.maxsalary" />
      </el-form-item>
      <el-form-item>
...
const viewData = async (id: string) => {
  farm2Request.submit('api/authgrade/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/authgrade', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/authgrade/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
