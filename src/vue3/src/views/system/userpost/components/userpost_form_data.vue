<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false" size="500px">
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
          <el-form-item label="主鍵：" prop="id" v-if="isShowform.id" :rules="v.getRules(false)">
        <el-select v-model="form.id" placeholder="请选择">
          <el-option label="~" value="" />
        </el-select>
      </el-form-item>
      <el-form-item label="用户key:" prop="userkey" v-if="isShowform.userkey" :rules="v.getRules(true, 0, 32)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/userpost/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/userpost', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/userpost/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
