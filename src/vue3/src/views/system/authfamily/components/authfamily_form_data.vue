<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto"
      ...
      <el-form-item label="主鍵:" prop="id" v-if="isShowform.id && false" :rules="v.getRules(true, 0, 16)">
      ...
      <el-form-item label="编码:" prop="keyid" v-if="isShowform.keyid" :rules="v.getRules(false, 0, 16)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/authfamily/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/authfamily', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/authfamily/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
