...
const viewData = async (id: string) => {
  farm2Request.submit('api/authptype/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/authptype', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/authptype/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
