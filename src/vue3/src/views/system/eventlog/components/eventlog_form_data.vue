<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false" size="500px">
    ...
    <el-form v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      ...
      <el-form-item label="动作类型:" prop="actiontype" v-if="isShowform.actiontype" :rules="v.getRules(true, 0, 64)">
        <el-input v-model="form.actiontype" />
      </el-form-item>
      <el-form-item label="事件对象:" prop="objtype" v-if="isShowform.objtype" :rules="v.getRules(true, 0, 64)">
        <el-input v-model="form.objtype" />
      </el-form-item>
      <el-form-item label="对象ID：" prop="objid" v-if="isShowform.objid" :rules="v.getRules(false, 0, 64)">
        <el-input v-model="form.objid" autosize type="textarea" />
      </el-form-item>
      <el-form-item label="用户IP:" prop="userip" v-if="isShowform.userip" :rules="v.getRules(false, 0, 16)">
        <el-input v-model="form.userip" />
      </el-form-item>
      <el-form-item label="用户KEY:" prop="userkey" v-if="isShowform.userkey" :rules="v.getRules(false, 0, 32)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/eventlog/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/eventlog', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/eventlog/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
