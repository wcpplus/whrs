<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      <el-form-item label="参数名称：" prop="name" v-if="isShowform.name">
        {{ form.name }}
      </el-form-item>
      <el-form-item label="参数关键字:" prop="pkey" v-if="isShowform.pkey">
        {{ form.pkey }}
      </el-form-item>
      <el-form-item label="参数描述：" prop="describes" v-if="isShowform.describes">
        {{ form.describes }}
      </el-form-item>
      <el-form-item label="参数类型：" prop="vtype" v-if="isShowform.vtype">
        {{ form.vtype == '1' ? '文本' : '' }}
        {{ form.vtype == '2' ? '布尔' : '' }}
        {{ form.vtype == '3' ? '数值' : '' }}
        {{ form.vtype == '4' ? '大文本' : '' }}
      </el-form-item>
      <el-form-item label="参数值：" prop="pvalue" v-if="isShowform.pvalue" :rules="v.getRules(true)">
        <el-input v-if="form.vtype == '1'" v-model="form.pvalue" autosize type="textarea" />
        <el-input v-if="form.vtype == '4'" v-model="form.pvalue" :rows="7" type="textarea" />
        <el-switch v-if="form.vtype == '2'" v-model="form.pvalue" size="large" active-value="true"
          inactive-value="false" active-text="可用" inactive-text="禁用" />
        <el-input-number v-if="form.vtype == '3'" v-model="form.pvalue" />
      </el-form-item>
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/parameter/val', RequestTypeEnum.post, isLoading, { key: form.pkey, val: form.pvalue, note: form.note }).then(() => {
...
//------------------------------------------------------------------------------------
onMounted(() => {
  eventBus.on(MittKesEnum.openSetValueForm, (paras) => {//注册打开form表单方法
