...
//formatSOURCE
const formatSOURCE = (_row: never, _column: never, cellValue: string) => {
  const dic: { [key: string]: string } = {
    "2": "自定义",
    "1": "XML",
    "3": "过期XML",
...
const formatUSERABLE = (_row: never, _column: never, cellValue: string) => {
  const dic: { [key: string]: string } = {
    "1": "是",
    "0": "否",
  };
...
export default { formatSTATE, formatTime ,formatSOURCE,formatUSERABLE};
