<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    size="500px">
    <div class="title">{{ currentFormType.type.title }}机构</div>
    <el-form v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      <el-form-item label="上级机构:" v-if="isShowform.parentLable">
        {{ parentLable }}
      </el-form-item>
      <el-form-item label="ID:" prop="id" v-if="isShowform.id" :rules="v.getRules(true, 0, 32)">
        <el-input v-model="form.id" />
      </el-form-item>
      <el-form-item label="名称:" prop="name" v-if="isShowform.name" :rules="v.getRules(true, 0, 32)">
      ...
      <el-form-item label="父节点:" prop="parentid" v-if="isShowform.parentid" :rules="v.getRules(true, 0, 32)">
      ...
      <el-form-item label="编码:" prop="code" v-if="isShowform.code" :rules="v.getRules(true, 0, 256)">
        <el-input v-model="form.code" autosize type="textarea" />
      </el-form-item>
      <el-form-item>
...
const viewData = async (id: string) => {
  farm2Request.submit('api/localorg/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/localorg', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/localorg/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
