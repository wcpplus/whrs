<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      ...
      <el-form-item label="菜单关键字：" prop="menukey" v-if="form.type === '1'" :rules="v.getRules(false, 0, 256)">
        <el-input v-model="form.menukey" autosize type="textarea" />
      </el-form-item>
      <el-form-item label="权限关键字：" prop="actions" v-if="form.type === '1' || form.type === '2'"
        :rules="v.getRules(false, 0, 256)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/actions/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/actions', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/actions/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
