<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false" size="500px">
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      ...
      <el-form-item label="计薪周期:" prop="salarytime" v-if="isShowform.salarytime" :rules="v.getRules(true, 0, 4)">
        <el-input v-model="form.salarytime" />
      </el-form-item>
      <el-form-item label="USERKEY:" prop="userkey" v-if="isShowform.userkey" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.userkey" />
      </el-form-item>
      <el-form-item label="USERNAME:" prop="username" v-if="isShowform.username" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.username" />
      </el-form-item>
      <el-form-item label="创建时间:" prop="ctime" v-if="isShowform.ctime" :rules="v.getRules(true, 0, 7)">
        <el-input v-model="form.ctime" />
      </el-form-item>
      <el-form-item label="备注：" prop="note" v-if="isShowform.note" :rules="v.getRules(false, 0, 128)">
      ...
      <el-form-item label="应发工资:" prop="grosspay" v-if="isShowform.grosspay" :rules="v.getRules(true)">
        <el-input v-model="form.grosspay" />
      </el-form-item>
      <el-form-item label="实发工资:" prop="netpay" v-if="isShowform.netpay" :rules="v.getRules(true)">
        <el-input v-model="form.netpay" />
      </el-form-item>
      <el-form-item label="个人所得税:" prop="taxamount" v-if="isShowform.taxamount" :rules="v.getRules(true)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/salaryuserunit/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/salaryuserunit', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/salaryuserunit/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
