<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false" size="500px">
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      ...
      <el-form-item label="KEYCODE:" prop="keycode" v-if="isShowform.keycode" :rules="v.getRules(true, 0, 16)">
      ...
      <el-form-item label="VAL:" prop="val" v-if="isShowform.val" :rules="v.getRules(true)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/salaryuseritem/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/salaryuseritem', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/salaryuseritem/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
