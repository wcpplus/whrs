<template>
  <el-dialog v-if="state.dialogVisible" :append-to-body="true" style="background-color: rgb(248, 248, 250);"
    v-model="state.dialogVisible" width="1000">
          ...
          <el-container>
            <el-header>
              ...
              <div class="farm2-data-buttons-operators">
                <el-button icon="Plus" type="primary" @click="openDataFormClick" plain>添加计算步骤</el-button>
                <el-button icon="GobletSquareFull" type="success" @click="runAllFormulaClick" plain>模拟计算薪资</el-button>
            ...
            <el-main>
              ...
              <div class="farm2-data-table">
                <el-table v-loading="loadingFlag" :data="dataResult.data" :stripe="true"
                  ...
                  <el-table-column v-if="false" prop="NAME" label="名称" min-width="150" />
                  <el-table-column v-if="false" sortable="custom" prop="RULEVAL" label="公式" min-width="150" />
                  <el-table-column v-if="false" prop="VALNAME" label="结果名称" min-width="120" />
                  <el-table-column v-if="false" prop="TEMPLATEID" label="TEMPLATEID" min-width="120" />
                  <el-table-column v-if="false" prop="SHOWMODEL" label="是否显示" min-width="100" />
                  <el-table-column v-if="false" prop="SORTCODE" label="排序" min-width="120" />
                  <el-table-column prop="STEPCODE" label="步骤" min-width="80">
                    <template #default="scope">
                      第 {{ scope.row.STEPCODE }}步
                    </template>
                  </el-table-column>
                  <el-table-column prop="VALCODE_ALT" label="计算结果" min-width="120" />
                  <el-table-column prop="VALCODE" label="关键字" min-width="120" />
    ...
    <!--高级查询表单-->
    <SalaryTemplateFormula_form_query></SalaryTemplateFormula_form_query>
    <!--数据实体表单-->
    <SalaryTemplateFormula_form_data></SalaryTemplateFormula_form_data>
  </el-dialog>
</template>
...
import SalaryTemplateFormula_form_query from './components/salarytemplateformula_form_query.vue';
import SalaryTemplateFormula_form_data from './components/salarytemplateformula_form_data.vue';
...
//---------------------------------------------------------
//路由信息：
//import salarytemplateformula_page from "@/views/system/salarytemplateformula/salarytemplateformula_page.vue";
// {
//     path: "salarytemplateformula",
//     name: "salarytemplateformula_page",
//     component: salarytemplateformula_page,
//     meta: { title: "计算规则" },
//   },
//---------------------------------------------------------
...
const doDataQuery = async (query?: DataQuery) => {
  ...
  QueryUtils.initRule(dataQuery, "TEMPLATEID", id2Ref.value, "=");
  farm2Request.search('api/salarytemplateformula/query', dataQuery, loadingFlag)
...
const doDataDelete = (ids: string[]) => {
  Farm2Request.submit(
    'api/salarytemplateformula/batch',
 ...
 * @param row
 */
const runAllFormulaClick = () => {
  farmClientUtils.postObject('api/salarytemplateformula/runAll', { key: id2Ref.value }, (obj: object) => {
...
const dataViewClick = (row: { ID: string }) => {
  eventBus.emit(PageEvent.data_form_open, { isOpen: true, type: <FormParam>{ type: getViewType(), id: row.ID, data: row, id2: id2Ref.value } });
}
