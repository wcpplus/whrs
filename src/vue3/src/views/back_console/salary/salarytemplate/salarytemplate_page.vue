<template>
          ...
          <el-main>
            ...
            <div class="farm2-data-table">
              <el-table v-loading="loadingFlag" :data="dataResult.data" :stripe="true"
                ...
                <el-table-column sortable="custom" prop="ID" label="主鍵" v-if="false" min-width="120" />
                <el-table-column sortable="custom" prop="CUSER" label="创建用户" v-if="false" min-width="120" />
                <el-table-column sortable="custom" prop="CTIME" label="创建时间" v-if="false" min-width="120" />
                <el-table-column fixed="left" sortable="custom" prop="NAME" label="名称" width="250" />
                <el-table-column v-if="false" label="数据项" width="200">
                  <template #default="scope">
                    <el-button link type="primary" size="small" @click="itemWinRef.openWin(true, scope.row.ID)">
                      数据项(<el-text class="mx-1" type="success">{{ scope.row.ITEM_NUM_ALT }}</el-text> )
                    </el-button>
                  </template>
                </el-table-column>
                <el-table-column label="计算规则" width="200">
                  <template #default="scope">
                    <el-button link type="primary" size="small" @click="formulaWinRef.openWin(true, scope.row.ID)">
                      计算规则(<el-text class="mx-1" type="success">{{ scope.row.FORMULA_NUM_ALT }}</el-text> )
                    </el-button>
                  </template>
                </el-table-column>
                <el-table-column sortable="custom" prop="STATE" label="状态" :formatter="dic.formatSTATE"
                  min-width="100" />
                <el-table-column sortable="custom" prop="NOTE" label="备注" min-width="300" />
                <el-table-column fixed="right" label="操作" width="240">
                  <template #default="scope">
                    <el-button link v-if="false" type="primary" size="small" @click="dataViewClick(scope.row)">
                      查看
                    </el-button>
                    ...
                    <el-button link type="danger" size="small" @click="dataDeleteClick(scope.row)">删除</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
  ...
  <Salarytemplateitem_win ref="itemWinRef"></Salarytemplateitem_win>
  <Salarytemplateformula_win ref="formulaWinRef"></Salarytemplateformula_win>
  <!--高级查询表单-->
  <SalaryTemplate_form_query></SalaryTemplate_form_query>
  <!--数据实体表单-->
  <SalaryTemplate_form_data></SalaryTemplate_form_data>
</template>
...
import SalaryTemplate_form_query from './components/salarytemplate_form_query.vue';
import SalaryTemplate_form_data from './components/salarytemplate_form_data.vue';
...
import Salarytemplateformula_win from '../salarytemplateformula/salarytemplateformula_win.vue';
//---------------------------------------------------------
//路由信息：
//import salarytemplate_page from "@/views/system/salarytemplate/salarytemplate_page.vue";
// {
//     path: "salarytemplate",
//     name: "salarytemplate_page",
//     component: salarytemplate_page,
//     meta: { title: "薪酬模板" },
//   },
//---------------------------------------------------------
const itemWinRef = ref();
const formulaWinRef = ref();
...
const doDataQuery = async (query?: DataQuery) => {
  ...
  farm2Request.search('api/salarytemplate/query', dataQuery, loadingFlag)
