<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto"
      ...
      <el-form-item label="创建用户:" prop="cuser" v-if="isShowform.cuser" :rules="v.getRules(false, 0, 16)">
        <el-input v-model="form.cuser" />
      </el-form-item>
...
const viewData = async (id: string) => {
  farm2Request.submit('api/salarytemplate/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/salarytemplate', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/salarytemplate/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
