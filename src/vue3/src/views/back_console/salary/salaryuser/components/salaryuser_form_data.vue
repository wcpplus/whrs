<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    size="500px">
    <div class="title">{{ currentFormType.type.title }}-计算模板</div>
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto"
      ...
      <el-form-item label="薪酬计算模板:" prop="templateid" v-if="isShowform.templateid" :rules="v.getRules(false, 0, 32)">
        <el-select v-model="form.templateid" filterable placeholder="选择">
...
import type { SalaryTemplate } from '../../salarytemplate/utils/salarytemplate';
...
const templates: Array<SalaryTemplate> = reactive([])
...
const drawerWinOpen = (isOpen: boolean, formParam?: FormParam) => {
  if (!isOpen) {
    ...
    farmClientUtils.postList('api/salarytemplate/all', {}, (list: []) => {
...
const viewData = async (id: string) => {
  farm2Request.submit('api/salaryuser/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/salaryuser', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/salaryuser/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
