 ...
 *
 *  import dic from './utils/dictinarys'
 * 转义 用于el-table-column 的 :formatter="dic.formatREAD" 转义数据列表数据值
 * @param _row
 * @param _column
 * @param cellValue
 * @returns
 */
const formatREAD = (_row: never, _column: never, cellValue: string) => {
  const dic: { [key: string]: string } = {
    "0": "未读",
    "1": "已读",
  };
...
export default { formatSTATE, formatTime ,formatREAD};
