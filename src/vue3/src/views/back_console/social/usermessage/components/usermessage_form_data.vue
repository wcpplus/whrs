<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto"
      ...
      <el-form-item label="主题：" prop="title" v-if="isShowform.title" :rules="v.getRules(true, 0, 128)">
        <el-input v-model="form.title" autosize type="textarea" />
      </el-form-item>
      <el-form-item label="阅读人:" prop="readuserkey" v-if="isShowform.readuserkey" :rules="v.getRules(true, 0, 512)">
        <el-input v-model="form.readuserkey" style="width: 220px;" autosize type="textarea" />
        <el-button v-if="formCtrl.isAbledForm" style="margin-left: 10px;" type="primary" @click="openUserChoose"
          round>选择用户</el-button>
      </el-form-item>
      <el-form-item label="正文：" prop="content" v-if="isShowform.content" :rules="v.getRules(true, 0, 512)">
        <el-input v-model="form.content" autosize type="textarea" />
      </el-form-item>
      <el-form-item label="阅读状态：" prop="readstate" v-if="isShowform.readstate" :rules="v.getRules(true)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/usermessage/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/usermessage', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/usermessage/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
