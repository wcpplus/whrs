<template>
      ...
      <el-container>
        <el-container>
          <el-header>
            ...
            <Data_querys placeholder="附件名称或ID" :isShowQueryPlus="true">
            </Data_querys>
          ...
          <el-main>
            ...
            <div class="farm2-data-table">
              <el-table v-loading="loadingFlag" :data="dataResult.data" :stripe="true"
                ...
                <el-table-column sortable="custom" prop="TITLE" label="文件名称" min-width="340" />
                <el-table-column sortable="custom" prop="PSTATE" label="状态" :formatter="dic.formatSTATE"
                  min-width="100" />
                <el-table-column sortable="custom" prop="ETIME" label="更新时间" :formatter="dic.formatTime"
                  min-width="160" />
                <el-table-column sortable="custom" prop="CTIME" label="创建时间" :formatter="dic.formatTime"
                  min-width="160" />
                <el-table-column sortable="custom" prop="FILEID" label="文件ID" min-width="280" />
                <el-table-column sortable="custom" prop="SERVERID" label="服务器ID" min-width="120" />
                <el-table-column fixed="right" label="操作" min-width="250">
                  <template #default="scope">
                    ...
                    <el-button link type="success" icon="CaretRight" size="small"
                      @click="dataRunClick(scope.row)">立即转换</el-button>
  ...
  <!--高级查询表单-->
  <WdapTask_form_query></WdapTask_form_query>
  <!--数据实体表单-->
  <WdapTask_form_data></WdapTask_form_data>
</template>
...
import WdapTask_form_query from './components/wdaptask_form_query.vue';
import WdapTask_form_data from './components/wdaptask_form_data.vue';
...
//---------------------------------------------------------
//路由信息：
//import wdaptask_page from "@/views/system/wdaptask/wdaptask_page.vue";
// {
//     path: "wdaptask",
//     name: "wdaptask_page",
//     component: wdaptask_page,
//     meta: { title: "转换任务" },
//   },
//---------------------------------------------------------
...
const doDataQuery = async (query?: DataQuery) => {
  farm2Request.search('api/wdaptask/query', Object.assign(dataQuery, query), loadingFlag)
 ...
 * @param row
 */
const dataRunClick = (row: { ID: string }) => {
  ElMessageBox.confirm('确认执行?', { icon: 'QuestionFilled' })
    .then(() => {
      Farm2Request.submit(
        'api/wdaptask/run',
        RequestTypeEnum.post,
        loadingFlag, { id: row.ID }
      ).then(() => {
        doDataQuery();
      }).catch((msg: string) => {
        notice.byError(msg);
