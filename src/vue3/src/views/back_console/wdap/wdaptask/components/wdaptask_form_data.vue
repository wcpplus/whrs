<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto"
      ...
      <el-form-item label="文件ID:" prop="fileid" v-if="isShowform.fileid" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.fileid" />
      </el-form-item>
      <el-form-item label="状态：" prop="pstate" v-if="isShowform.pstate" :rules="v.getRules(true)">
        <el-select v-model="form.pstate" placeholder="请选择">
          <el-option label="~" value="" />
        </el-select>
      </el-form-item>
      <el-form-item label="转换日志：" prop="logjson" v-if="isShowform.logjson" :rules="v.getRules(true, 0, 327675)">
        <el-input v-model="form.logjson" autosize type="textarea" />
      </el-form-item>
      ...
      <el-form-item label="拓展文件夹:" prop="exPath" v-if="isShowform.exPath">
        <el-input v-model="form.exPath" autosize type="textarea" />
      </el-form-item>
      <el-form-item>
...
const viewData = async (id: string) => {
  farm2Request.submit('api/wdaptask/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/wdaptask', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/wdaptask/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
