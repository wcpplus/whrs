<template>
      ...
      <el-container>
        <el-container>
          <el-header>
            ...
            <Data_querys placeholder="流程或扩展名" :isShowQueryPlus="true">
            </Data_querys>
          ...
          <el-main>
            ...
            <div class="farm2-data-table">
              <el-table v-loading="loadingFlag" :data="dataResult.data" :stripe="true"
                ...
                <el-table-column sortable="custom" prop="NAME" label="流程名" min-width="220" />
                <el-table-column sortable="custom" prop="EXNAME" label="扩展名" min-width="120" />
                <el-table-column sortable="custom" prop="SIZEMIN" label="最小文件" :formatter="dic.formatFileSize"
                  min-width="120" />
                <el-table-column sortable="custom" prop="SIZEMAX" label="最大文件" :formatter="dic.formatFileSize"
                  min-width="120" />
                ...
                <el-table-column fixed="right" label="操作" min-width="280">
                  <template #default="scope">
                    ...
                    <el-button link type="success" icon="Share" size="small" @click="flowEditClick(scope.row)">
                      流程配置
                    </el-button>
  ...
  <!--高级查询表单-->
  <WdapFlow_form_query></WdapFlow_form_query>
  <!--数据实体表单-->
  <WdapFlow_form_data></WdapFlow_form_data>
  <!--流程设计-->
  <wdapflow_flow_design ref="flowWinRef"></wdapflow_flow_design>
</template>
<script setup lang="ts">
import wdapflow_flow_design from './components/wdapflow_flow_design.vue';
...
import WdapFlow_form_query from './components/wdapflow_form_query.vue';
import WdapFlow_form_data from './components/wdapflow_form_data.vue';
...
//---------------------------------------------------------
//路由信息：
//import wdapflow_page from "@/views/system/wdapflow/wdapflow_page.vue";
// {
//     path: "wdapflow",
//     name: "wdapflow_page",
//     component: wdapflow_page,
//     meta: { title: "转换流程" },
//   },
//---------------------------------------------------------
const eventBus = mitt();//事件总线
const flowWinRef = ref();
...
const doDataQuery = async (query?: DataQuery) => {
  farm2Request.search('api/wdapflow/query', Object.assign(dataQuery, query), loadingFlag)
 ...
 * @param row 选定行数据
 */
const flowEditClick = (row: { ID: string }) => {
  flowWinRef.value.openWin(row.ID);
}
