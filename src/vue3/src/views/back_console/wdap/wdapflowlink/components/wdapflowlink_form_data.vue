<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false" size="500px">
    ...
    <el-form v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      ...
      <el-form-item label="源节点ID:" prop="snodeid" v-if="isShowform.snodeid" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.snodeid" />
      </el-form-item>
      <el-form-item label="目标节点ID:" prop="tnodeid" v-if="isShowform.tnodeid" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.tnodeid" />
      </el-form-item>
      <el-form-item label="流程ID:" prop="flowid" v-if="isShowform.flowid" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.flowid" />
      </el-form-item>
      <el-form-item label="布尔表达式：" prop="expression" v-if="isShowform.expression" :rules="v.getRules(true, 0, 512)">
        <el-input v-model="form.expression" autosize type="textarea" />
      </el-form-item>
      <el-form-item>
...
const viewData = async (id: string) => {
  farm2Request.submit('api/wdapflowlink/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/wdapflowlink', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/wdapflowlink/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
