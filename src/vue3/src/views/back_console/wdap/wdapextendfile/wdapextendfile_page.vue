<template>
          ...
          <el-main>
            ...
            <div class="farm2-data-table">
              <el-table v-loading="loadingFlag" :data="dataResult.data" :stripe="true"
                ...
                <el-table-column sortable="custom" prop="TITLE" label="附件名称" min-width="160" />
                <el-table-column sortable="custom" prop="FILEID" label="附件ID" min-width="60" />
                <el-table-column sortable="custom" prop="MODELTITLE" label="附件模型" min-width="80" />
                <el-table-column sortable="ctime" prop="CTIME" label="创建时间"  :formatter="dic.formatTime"   min-width="80" />
                <el-table-column sortable="custom" prop="RESOURCEKEY" label="资源KEY" min-width="80" />
                <el-table-column sortable="custom" prop="SERVERID" label="服务器ID" min-width="80" />
                <el-table-column fixed="right" label="操作" min-width="120">
                  <template #default="scope">
                    <el-button link type="primary" icon="View" size="small" @click="dataViewClick(scope.row)">
                      查看
                    </el-button>
                    <el-button link type="success" icon="View" size="small" @click="showViewClick(scope.row)">
                      预览
                    </el-button>
  ...
  <!--高级查询表单-->
  <WdapExtendFile_form_query></WdapExtendFile_form_query>
  <!--数据实体表单-->
  <WdapExtendFile_form_data></WdapExtendFile_form_data>
  <!--文件预览窗口-->
  <wdapextendfile_form_view ref="viewWin"></wdapextendfile_form_view>
</template>
<script setup lang="ts">
import Data_navigate from '@/components/datas/data_navigate.vue';
import wdapextendfile_form_view from './components/wdapextendfile_form_view.vue';
...
import WdapExtendFile_form_query from './components/wdapextendfile_form_query.vue';
import WdapExtendFile_form_data from './components/wdapextendfile_form_data.vue';
...
//---------------------------------------------------------
//路由信息：
//import wdapextendfile_page from "@/views/system/wdapextendfile/wdapextendfile_page.vue";
// {
//     path: "wdapextendfile",
//     name: "wdapextendfile_page",
//     component: wdapextendfile_page,
//     meta: { title: "扩展附件" },
//   },
//---------------------------------------------------------
...
const doDataQuery = async (query?: DataQuery) => {
  farm2Request.search('api/wdapextendfile/query', Object.assign(dataQuery, query), loadingFlag)
 ...
 * @param row 选定行数据
 */
const showViewClick = (row: { ID: string }) => {
  viewWin.value.openWin(row);
}
