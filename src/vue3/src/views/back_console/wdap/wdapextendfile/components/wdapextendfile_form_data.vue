<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false"
    ...
    <el-form :show-message="formCtrl.isShowValidMsg" v-loading="isLoading" :model="form" label-width="auto"
      ...
      <el-form-item label="任务ID:" prop="taskid" v-if="isShowform.taskid" :rules="v.getRules(false, 0, 16)">
        <el-input v-model="form.taskid" />
      </el-form-item>
      <el-form-item label="附件ID:" prop="fileid" v-if="isShowform.fileid" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.fileid" />
      </el-form-item>
      <el-form-item label="附件模型:" prop="filemodel" v-if="isShowform.filemodel" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.filemodel" />
      </el-form-item>
      <el-form-item label="是否可预览：" prop="viewis" v-if="isShowform.viewis" :rules="v.getRules(true)">
        <el-select v-model="form.viewis" placeholder="请选择">
      ...
      <el-form-item label="资源目录KEY:" prop="resourcekey" v-if="isShowform.resourcekey" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.resourcekey" />
      </el-form-item>
      <el-form-item label="服务器ID:" prop="serverid" v-if="isShowform.serverid" :rules="v.getRules(true, 0, 16)">
        <el-input v-model="form.serverid" />
      </el-form-item>
      <el-form-item label="扩展文件目录:" prop="dirpath" v-if="isShowform.dirpath">
        <el-input v-model="form.dirpath"  autosize type="textarea" />
      </el-form-item>
      <el-form-item>
...
const viewData = async (id: string) => {
  farm2Request.submit('api/wdapextendfile/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/wdapextendfile', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/wdapextendfile/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
