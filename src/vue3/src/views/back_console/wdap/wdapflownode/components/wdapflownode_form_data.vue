<template>
  <el-drawer v-model="formCtrl.isDrawerOpen" class="farm2-data-form" :append-to-body="true" :with-header="false" size="500px">
    ...
    <el-form v-loading="isLoading" :model="form" label-width="auto" ref="ruleFormRef" :disabled="!formCtrl.isAbledForm">
      ...
      <el-form-item label="绑定业务模型：" prop="model" v-if="isShowform.model" :rules="v.getRules(true, 0, 64)">
        <el-input v-model="form.model" autosize type="textarea" />
      </el-form-item>
      <el-form-item label="绑定业务模型ID:" prop="rid" v-if="isShowform.rid" :rules="v.getRules(false, 0, 16)">
...
const viewData = async (id: string) => {
  farm2Request.submit('api/wdapflownode/' + id, RequestTypeEnum.get, isLoading).then((response: DataResponse) => {
...
const addData = async () => {
  ...
  if (currentFormType.type.key == getCreatType().key) {
    farm2Request.submit('api/wdapflownode', RequestTypeEnum.post, isLoading, form).then(() => {
  ...
  if (currentFormType.type.key == getUpdateType().key) {
    farm2Request.submit('api/wdapflownode/' + form.id, RequestTypeEnum.put, isLoading, form).then(() => {
