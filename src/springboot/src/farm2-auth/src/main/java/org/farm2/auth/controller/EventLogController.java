...
import org.farm2.auth.dto.EventLogDto;
...
public class EventLogController {
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('eventLog.query')")
    @PostMapping("/query")
    public FarmResponseResult queryAll(@RequestBody DataQueryDto query) {
        ...
        dataQuery.setRuleAsSql("default", "and (NOTE like ?)", FarmDbLikeModel.ALL);
        DataResult result = eventLogServiceImpl.searchEventLog(dataQuery);
    ...
    public FarmResponseResult queryAll(@RequestBody DataQueryDto query) {
        ...
        result.runDataHandle(new ResultDataHandle() {
            ...
            public void handle(Map<String, Object> row) {
                row.put("isLeaf", eventLogServiceImpl.getNum(DataQuery.getInstance().addRule(new DBRule("PARENTID", row.get("ID"), "="))) <= 0);
            }
        });
        return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, result);
    }    
    */
    ...
    public FarmResponseResult treeload(@RequestBody DataQueryDto query) {
        ...
        if (parentid == null || StringUtils.isBlank(parentid.toString())) {
            ...
            node.put("NAME", "事件日志");
            node.put("isLeaf", eventLogServiceImpl.getNum(DataQuery.getInstance().addRule(new DBRule("PARENTID", "NONE", "="))) <= 0);
            list.add(node);
    ...
    */
    ...
    @PreAuthorize("@farmAction.has('eventLog.move')")
    @PostMapping("/move")
    public FarmResponseResult move(@RequestBody IdAndIdsDto paras) {
        eventLogServiceImpl.moveTreeNode(paras.getIds(), paras.getId());
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('eventLog.add')")
    @PostMapping
    public FarmResponseResult addSubmit(@Valid @RequestBody EventLogDto eventLogDto) {
        EventLog eventLog = eventLogServiceImpl.insertEventLogEntity(FarmBeanUtils.copyProperties(eventLogDto, new EventLog()));
        return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, eventLog.getId());
    }
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('eventLog.edit')")
    @PutMapping("/{id}")
    public FarmResponseResult editSubmit(@PathVariable String id, @Valid @RequestBody EventLogDto eventLogDto) {
        eventLogDto.setId(id);
        eventLogServiceImpl.editEventLogEntity(FarmBeanUtils.copyProperties(eventLogDto, new EventLog()));
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('eventLog.del')")
    @DeleteMapping("/{id}")
    public FarmResponseResult delSubmit(@PathVariable String id) {
        eventLogServiceImpl.delEventLog(id);
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('eventLog.info')")
    @GetMapping("/{id}")
    public FarmResponseResult info(@PathVariable String id) {
        EventLog eventLog = eventLogServiceImpl.getEventLogById(id);
        return new FarmResponseResult(FarmResponseCode.SUCESS, FarmBeanUtils.copyProperties(eventLog, new EventLogDto()));
    }
}
