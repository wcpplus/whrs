...
import org.farm2.wdap.dto.WdapFlowNodeDto;
 ...
 */
@RestController
@RequestMapping("/api/wdapflownode")
public class WdapFlowNodeController {
    @Autowired
    private WdapFlowNodeServiceInter wdapFlowNodeServiceImpl;
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('wdapFlowNode.query')")
    @PostMapping("/query")
    public FarmResponseResult queryAll(@RequestBody DataQueryDto query) {
        ...
        DataResult result = wdapFlowNodeServiceImpl.searchWdapFlowNode(dataQuery);
    ...
    public FarmResponseResult queryAll(@RequestBody DataQueryDto query) {
        ...
        result.runDataHandle(new ResultDataHandle() {
            ...
            public void handle(Map<String, Object> row) {
                row.put("isLeaf", wdapFlowNodeServiceImpl.getNum(DataQuery.getInstance().addRule(new DBRule("PARENTID", row.get("ID"), "="))) <= 0);
            }
        });
        return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, result);
    }    
    */
    ...
    public FarmResponseResult treeload(@RequestBody DataQueryDto query) {
        ...
        if (parentid == null || StringUtils.isBlank(parentid.toString())) {
            ...
            node.put("NAME", "流程节点");
            node.put("isLeaf", wdapFlowNodeServiceImpl.getNum(DataQuery.getInstance().addRule(new DBRule("PARENTID", "NONE", "="))) <= 0);
            list.add(node);
    ...
    */
    ...
    @PreAuthorize("@farmAction.has('wdapFlowNode.move')")
    @PostMapping("/move")
    public FarmResponseResult move(@RequestBody IdAndIdsDto paras) {
        wdapFlowNodeServiceImpl.moveTreeNode(paras.getIds(), paras.getId());
    ...
    @PreAuthorize("@farmAction.has('wdapFlowNode.autosort')")
    @PostMapping("/autosort")
    public FarmResponseResult autosort(@RequestBody List<String> ids) {
        wdapFlowNodeServiceImpl.autoSort(ids);
        return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, null);
    }*/
     ...
     * 添加数据
     *
     * @param wdapFlowNodeDto
     * @return
     */
    @PreAuthorize("@farmAction.has('wdapFlowNode.add')")
    @PostMapping
    public FarmResponseResult addSubmit(@Valid @RequestBody WdapFlowNodeDto wdapFlowNodeDto) {
        WdapFlowNode wdapFlowNode = wdapFlowNodeServiceImpl.insertWdapFlowNodeEntity(FarmBeanUtils.copyProperties(wdapFlowNodeDto, new WdapFlowNode()));
        return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, wdapFlowNode.getId());
    }
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('wdapFlowNode.edit')")
    @PutMapping("/{id}")
    public FarmResponseResult editSubmit(@PathVariable String id,@RequestBody WdapFlowNodeDto wdapFlowNodeDto) {
        WdapFlowNode node = wdapFlowNodeServiceImpl.getWdapFlowNodeById(id);
        node.setTimeout(wdapFlowNodeDto.getTimeout());
        wdapFlowNodeServiceImpl.editWdapFlowNodeEntity(node);
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('wdapFlowNode.del')")
    @DeleteMapping("/{id}")
    public FarmResponseResult delSubmit(@PathVariable String id) {
        wdapFlowNodeServiceImpl.delWdapFlowNode(id);
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('wdapFlowNode.info')")
    @GetMapping("/{id}")
    public FarmResponseResult info(@PathVariable String id) {
        WdapFlowNode wdapFlowNode = wdapFlowNodeServiceImpl.getWdapFlowNodeById(id);
        return new FarmResponseResult(FarmResponseCode.SUCESS, FarmBeanUtils.copyProperties(wdapFlowNode, new WdapFlowNodeDto()));
    }
}
