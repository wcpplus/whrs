...
import org.farm2.salary.dto.SalaryTemplateItemDto;
 ...
 */
@RestController
@RequestMapping("/api/salarytemplateitem")
public class SalaryTemplateItemController {
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('salaryTemplateItem.query')")
    @PostMapping("/query")
    public FarmResponseResult queryAll(@RequestBody DataQueryDto query) {
        ...
        dataQuery.setRuleAsSql("default", "and (NAME like ?||COMPONENTTYPE like ?)", FarmDbLikeModel.ALL);
        DataResult result = salaryTemplateItemServiceImpl.searchSalaryTemplateItem(dataQuery);
        return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, result);
    }

    @PostMapping("/templateItems")
    public FarmResponseResult templateItems(@RequestBody IdDto temId) {
        List<SalaryTemplateItem> items = salaryTemplateItemServiceImpl.getSalaryTemplateItems(DataQuery.getInstance()
                .setPageSize(100)
                .addRule(new DBRule("TEMPLATEID", temId.getId(), "=")));
        return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, items);
    }
     ...
     * @return
     ...
     row.put("isLeaf", salaryTemplateItemServiceImpl.getNum(DataQuery.getInstance().addRule(new DBRule("PARENTID", row.get("ID"), "="))) <= 0);
     }
     });
     return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, result);
     }
     ...
     * @return
     ...
     node.put("NAME", "薪酬项");
     node.put("isLeaf", salaryTemplateItemServiceImpl.getNum(DataQuery.getInstance().addRule(new DBRule("PARENTID", "NONE", "="))) <= 0);
     list.add(node);
     ...
     * @return
     ...
     @PreAuthorize("@farmAction.has('salaryTemplateItem.move')")
     @PostMapping("/move") public FarmResponseResult move(@RequestBody IdAndIdsDto paras) {
     salaryTemplateItemServiceImpl.moveTreeNode(paras.getIds(), paras.getId());
     ...
     * @return
     ...
     @PreAuthorize("@farmAction.has('salaryTemplateItem.autosort')")
     @PostMapping("/autosort") public FarmResponseResult autosort(@RequestBody List<String> ids) {
     salaryTemplateItemServiceImpl.autoSort(ids);
     return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, null);
     }*/
     ...
     * 添加数据
     *
     * @param salaryTemplateItemDto
     * @return
     */
    @PreAuthorize("@farmAction.has('salaryTemplateItem.add')")
    @PostMapping
    public FarmResponseResult addSubmit(@Valid @RequestBody SalaryTemplateItemDto salaryTemplateItemDto) {
        SalaryTemplateItem salaryTemplateItem = salaryTemplateItemServiceImpl.insertSalaryTemplateItemEntity(FarmBeanUtils.copyProperties(salaryTemplateItemDto, new SalaryTemplateItem()));
        return FarmResponseResult.getInstance(FarmResponseCode.SUCESS, salaryTemplateItem.getId());
    }
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('salaryTemplateItem.edit')")
    @PutMapping("/{id}")
    public FarmResponseResult editSubmit(@PathVariable String id, @Valid @RequestBody SalaryTemplateItemDto salaryTemplateItemDto) {
        salaryTemplateItemDto.setId(id);
        salaryTemplateItemServiceImpl.editSalaryTemplateItemEntity(FarmBeanUtils.copyProperties(salaryTemplateItemDto, new SalaryTemplateItem()));
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('salaryTemplateItem.del')")
    @DeleteMapping("/{id}")
    public FarmResponseResult delSubmit(@PathVariable String id) {
        salaryTemplateItemServiceImpl.delSalaryTemplateItem(id);
     ...
     * @return
     */
    @PreAuthorize("@farmAction.has('salaryTemplateItem.info')")
    @GetMapping("/{id}")
    public FarmResponseResult info(@PathVariable String id) {
        SalaryTemplateItem salaryTemplateItem = salaryTemplateItemServiceImpl.getSalaryTemplateItemById(id);
        return new FarmResponseResult(FarmResponseCode.SUCESS, FarmBeanUtils.copyProperties(salaryTemplateItem, new SalaryTemplateItemDto()));
    }
}
